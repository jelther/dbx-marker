...
ON target.pipeline_name = source.pipeline_name AND target.checkpoint_type = source.checkpoint_type
WHEN MATCHED THEN
    UPDATE SET target.value = source.value, target.updated_at = source.updated_at
WHEN NOT MATCHED THEN
    INSERT (pipeline_name, checkpoint_type, value, updated_at)
    VALUES (source.pipeline_name, source.checkpoint_type, source.value, source.updated_at)
"""
