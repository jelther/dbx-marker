...
from dbx_marker.exceptions import MarkerExistsError, MarkerNotFoundError, MarkerUpdateError, MarkerDeleteError
from dbx_marker.sqls import INITIALIZE_TABLE_SQL, GET_MARKER_SQL, UPDATE_MARKER_SQL, DELETE_MARKER_SQL
...
class DbxMarkerManager:
    ...
    def _initialize_table(self):
        ...
        try:
            ...
            self.spark.sql(INITIALIZE_TABLE_SQL.format(delta_table_path=self.delta_table_path))
    ...
    def get_marker(self, pipeline_name: str, checkpoint_type: str = "marker") -> Optional[str]:
        """
        Get the current marker or checkpoint value for a given pipeline.

        :param pipeline_name: Unique identifier for the pipeline.
        :param checkpoint_type: The type of marker, either 'marker' (default) or 'checkpoint'.
        ...
        try:
            ...
            if df.count() == 0:
                logger.debug(f"No {checkpoint_type} found for pipeline {pipeline_name}.")
                raise MarkerNotFoundError(f"No {checkpoint_type} found for pipeline '{pipeline_name}'.")
        except MarkerNotFoundError as mnfe:
            logger.error(mnfe)
            raise mnfe
        except Exception as e:
            logger.error(f"Failed to retrieve {checkpoint_type} for pipeline {pipeline_name}: {e}")
            raise MarkerNotFoundError(
                f"An error occurred while retrieving {checkpoint_type} for pipeline '{pipeline_name}': {e}"
            ) from e
    ...
    def update_marker(self, pipeline_name: str, value: str) -> None:
        ...
        try:
            ...
            raise MarkerUpdateError(f"Failed to update marker for pipeline '{pipeline_name}': {e}") from e
    ...
    def delete_marker(self, pipeline_name: str) -> None:
        ...
        try:
            ...
            raise MarkerDeleteError(f"Failed to delete marker for pipeline '{pipeline_name}': {e}") from e